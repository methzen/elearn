...
export type CurrentChapterProps = { groupId: string, courseId: string, chapterId: string, sectionId: string }

export async function apiSetCurrentChapter(data: CurrentChapterProps){
  const token = localStorage.getItem('x-auth-token')
  const response = await axios.post(`/groups/course/current/chapter?groupId=${data.groupId}`, data, {
        headers: {
          "x-auth-token" : token,
        }
      })
